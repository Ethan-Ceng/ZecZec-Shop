<template>
	<div class="upload-wrap">
		...
		<div class="fileContainer">
			<div class="file-type">
				<ul>
					<li :class="activeType == item.category_id ? 'item active' : 'item'"
						v-for="(item, index) in typeList" :key="index" @click="selectTypeFunc(item.category_id)">
						{{ item.name }}
						<div class="operation" @click.stop v-if="item.category_id != null">
							<p @click="editCategoryFunc(item)">
								<el-icon>
									<Edit />
								</el-icon>
							</p>
							<p @click="deleteCategoryFunc(item.category_id)">
								<el-icon>
									<Delete />
								</el-icon>
							</p>
			...
			<div class="file-content">
				<ul class="fileContainerUI clearfix">
					<li class="file" v-for="(item, index) in fileList" :key="index"
						...
						<img v-if="this_config.fileType == 'image'" :scr="item.image"
							:style="'background-image:url(' + item.image + ')'" alt="" />
		...
		<!--图片类别-->
		<AddCategoryindex v-if="isShowCategory" :category="category" :fileType="this_config.fileType"
			@closeCategory="closeCategoryFunc"></AddCategoryindex>
	</div>
</template>

<script>
import FileApi from '@/api/file.js';
import AddCategoryindex from './part/AddCategory.vue';
export default {
	components: {
		AddCategoryindex
	},
	...
	methods: {
		...
		getFileCategory() {
			...
			FileApi.SystemPictureList(param, true)
				.then(res => {
					self.loading = false;
					self.fileList = res.data.list.data;
		...
		getData: function() {
			...
			true
				...
				.then(res => {
					...
					self.typeList = type.concat(res.data.list);
					self.category_id = self.typeList.category_id;
		...
		UploadImage(param) {
			let self = this;
			if (self.activeType == null || self.activeType == 0) {
			...
			formData.append('category_id', self.activeType == null ? 0 : self.activeType);
		...
		selectFileFunc(item, index) {
			...
			console.log('1', item.category_id);
		},
		...
		deleteFileFunc(e) {
			...
			ElMessageBox.confirm('此操作将永久删除该记录, 是否继续?', '提示', {
				...
				.catch(() => {
					loading && loading.close();
					console.log('id', self.category_id);
					ElMessage({
						message: '删除失败',
						type: 'warning'
					});
