...
function wxAppPay(result, self,success, fail){
	uni.requestPayment({
		provider: 'wxpay',
		orderInfo: {
			"appid": result.data.payment.appid,  // 微信开放平台 - 应用 - AppId，注意和微信小程序、公众号 AppId 可能不一致
			"noncestr": result.data.payment.noncestr, // 随机字符串
			"package": result.data.payment.package,        // 固定值
			"partnerid": ""+result.data.payment.partnerid,      // 微信支付商户号
			"prepayid": result.data.payment.prepayid, // 统一下单订单号 
			"timestamp": result.data.payment.timestamp,        // 时间戳（单位：秒）
			"sign": result.data.payment.sign // 签名，这里用的 MD5/RSA 签名
		},
		success(res) {
			paySuccess(result, self, success);
		},
