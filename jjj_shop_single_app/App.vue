			...
			plus.runtime.getProperty(plus.runtime.appid, function(widgetInfo) {
				uni.request({
					...
					success: data => {
						...
						if (result.update && result.wgtUrl) {
							self.updateWgt(result.wgtUrl);
						}
						if (result.update && result.pkgUrl) {
							plus.nativeUI.confirm(
								'有新版本更新，请点击确认更新到最新版本，以免影响使用',
								function(e) {
									if (e.index == 0) {
										plus.runtime.openURL(result.pkgUrl);
									}
								}, {
									title: '更新提示',
									buttons: ['确定', '取消'],
			...
			getWxopen() {
				...
				uni.request({
					...
					success: (result) => {
						...
						uni.setStorageSync('mpState', result.data.data.setting.mp_open);
						uni.setStorageSync('wxOpen', result.data.data.setting.wx_open);
						uni.setStorageSync('wxBinding', result.data.data.setting.wx_phone);
						uni.setStorageSync('smsOpen', result.data.data.setting.sms_open);
					}
				});
			},
			isFirstEnter() {
