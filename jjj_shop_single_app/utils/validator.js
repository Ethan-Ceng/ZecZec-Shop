...
function validator(app) {
	...
	app.config.globalProperties.getAppId = function() {
		return config.app_id || uni.getStorageSync('app_id') || 10001
	...
	app.config.globalProperties.topBarRight = function() {
		// #ifdef MP-WEIXIN
		let right = uni.getMenuButtonBoundingClientRect().right;
		let width = uni.getMenuButtonBoundingClientRect().width;
		let data = (width + 10) * 2
		return data + 'rpx';
		// #endif
		// #ifndef MP-WEIXIN
	 ...
	 * 订阅通知,目前仅公众号
	 */
	app.config.globalProperties.mpMessage = function(signPackage) {
		//#ifdef H5
		if (!signPackage) {
			return
		}
