	...
	<view :data-theme="theme()" :class="theme() || ''" class="coupon_bg" v-if="!loading">
		...
		<view class="titleBar">
			...
			<view class="tc d-c-c f32 pr" :style="'height:' + titleBarHeight + 'px;z-index:100'">
				<view class="reg180" @click="goback"><text class="icon iconfont icon-you"></text></view>
				<view>优惠券</view>
			</view>
					...
					<view :class="'coupon-item coupon-item-' + detail.color.text">
						<view class="operation d-b-c w-b">
							<view class="flex-1 coupon-content">
								...
								<view v-if="detail.coupon_type.value == 20" class=" f22">
									{{ detail.max_price > 0 ? '最多抵扣' + detail.max_price * 1 + '元' : '无最高抵扣限制' }}
								</view>
							</view>
							<view class="right-box d-c-c d-c">
								...
								<button type="default" v-else class="f26 coupon-btn btn-gray white">已领取</button>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="o-h pro_list" v-if="apply_range != 10">
				<view class="d-b-c f-w">
					<view class="pro_item" v-for="(item, index) in listData" :key="index"
						...
						<view class="f26 gray3 text-ellipsis mt20 tc">{{ item.product_name }}</view>
