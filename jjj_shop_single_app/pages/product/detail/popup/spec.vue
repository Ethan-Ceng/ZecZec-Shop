<template>
	<view :class="Visible ? 'product-popup open' : 'product-popup close'" @touchmove.stop.prevent=""
		...
		<view class="main" v-on:click.stop>
			<view class="header">
				<image @click="yulan(form.show_sku.sku_image,0)" :src="form.show_sku.sku_image" mode="aspectFit"
					class="avt"></image>
				<view class="price">
					¥
			...
			initShowSku() {
				...
				if (this.form.detail.spec_type == 20) {
					if (this.form.detail.product_price != this.form.detail.product_max_price) {
						this.form.show_sku.product_price = this.form.detail.product_price + '-' + this.form.detail
							.product_max_price;
					}
				}
				...
				if (this.form.plus_name == 'advance') {
					...
					this.form.show_sku.sku_image = this.form.plus_sku[0].productSku.image ? this.form.plus_sku[0]
						.productSku.image.file_path : this.form.detail.image[0].file_path;
			...
			updateSpecProduct() {
				...
				if (typeof skuItem === 'object') {
					...
					if (self.form.plus_name) {
						...
						self.form.show_sku.sku_image = skuItem.spec_form.image ? skuItem.spec_form.image.file_path : self
							.form.detail.image[0].file_path;
						self.form.show_sku.advance_product_id = skuItem.spec_form.image ? skuItem.spec_form.image
							.file_path : self.form.detail.image[0].file_path
					}
				}
			},
