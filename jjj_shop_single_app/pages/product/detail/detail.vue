<template>
	<view class="product-detail pr" :data-theme="theme()" :class="theme() || ''">
		...
		<scroll-view :scroll-top="topId" :scroll-with-animation="true" @scroll="scrollFunc" scroll-y="true"
			class="scroll-Y scroll-box" :style="'height:' + scrollviewHigh + 'px'" v-if="!loadding">
			<view class=" top-scroll-nav" :style="topBarHeight() == 0 ? '' : 'padding-top:' + topBarTop() + 'px'"
				:class="scrollId < 100 ? 'close' : 'open'">
				<!-- #ifdef MP-WEIXIN || APP-PLUS -->
				<view class="pr d-c-c">
					<view class="reg180" :style="topBarHeight() == 0 ? '' : 'height:' + topBarHeight() + 'px;'">
						<image @click="goback" src="/static/icon/back-jianatou.png" mode=""
				...
				<view class="d-b-c" style="height: 80rpx;">
					<view class="flex-1 tc f30 gray3 top-scroll-nav-item"
						:class="{ active: scrollId * 1 + 1 < commentTop }" @click="topId = 0">商品</view>
					<view class="flex-1 tc f30 gray3 top-scroll-nav-item"
					...
					<view class="flex-1 tc f30 gray3 top-scroll-nav-item"
						:class="{ active: scrollId * 1 + 1 > contentTop }" @click="topId = contentTop">详情</view>
				</view>
			</view>
			<!--图片-->
				...
				<view class="limited-spike m-0-20">
					<view class="d-s-c mb16">
						<view class="f32" v-if="activeName == 'advance'">
							￥
							<text
								class="f48 fb">{{ subPrice(detail[activeName][skuName][0].product_price, '1') }}.</text>
							<text
						...
						<view class="f32" v-else>
							...
							<text
								class="f48 fb">{{ subPrice(detail[activeName][skuName][0][activePrice], '1') }}.</text>
							<text class="f24 fb">{{ subPrice(detail[activeName][skuName][0][activePrice], '2') }}</text>
							<text
								class="text-d-line f24 ml10">￥{{ detail[activeName][skuName][0].product_price }}</text>
						</view>
						<view class="tips-box" v-if="activeName == 'advance'">
							预估到手价￥{{ (detail[activeName][skuName][0].product_price * 1 - detail[activeName][skuName][0][activePrice] * 1 + detail[activeName].money*1).toFixed(2) }}
						</view>
					</view>
					...
					<view class="d-e-c">
						<view class="flex-1">
							...
							<view class="f26 gray3 line-h-50 " v-if="discount.product_reduce.length > 0">
								<text class="manjian-box" v-for="(item, index) in discount.product_reduce" :key="index"
			...
			<!-- 普通商品 -->
			<view class="bg-white p30 mb22 product-info" v-if="!ispresale && is_preview != 1">
				<view class="price-wrap">
					<view class="d-s-s d-c ww100">
						<view class="d-s-c pr ww100 mb16">
							<view class="new-price theme-price">
								<text class="fn mr10 fb" v-if="detail.is_user_grade">会员价</text>
								<text>¥</text>
							...
							<view class="sc-box">
								<button class="d-c d-c-c" @click="favorite()">
									<image :class="is_fav ? '' : 'img_gray'" class="share_img" src="/static/icon/sc.png"
										mode=""></image>
			...
			<view class="o-h bg-white mb21 p-0-30 product-choice" v-if="detail.spec_type == 20">
				<!--已选择-->
				<view class="already-choice" @click="openPopup(ispresale ? 'deposit' : 'order')">
					<view class="group-hd">
						...
						<view class="right"><text class="icon iconfont icon-you"
			...
			<view class="product-comment" id="product-comment">
				<view class="p-0-30 d-b-c">
					<view class="group-hd">
						...
						<text
							class="f22 gray9">（{{ detail.comment_data_count > 99 ? '99+' : detail.comment_data_count }}）</text>
					</view>
					<view class="right" @click="lookEvaluate(detail.product_id)">
						<text class="f24 gray9">查看全部</text>
						<text class="icon iconfont icon-you ml20" style="font-size: 24rpx;color: #999;"></text>
					</view>
				</view>
				<view class="comment-list" v-if="detail.comment_data_count > 0">
					<view class="item" v-for="(item, index) in detail.commentData" :key="index">
						<view class="cmt-user">
							<view class="left">
								<image class="photo" :src="item.user ? item.user.avatarUrl : ''" mode="aspectFill">
								</image>
			...
			<view class="product-content" id="product-content">
				...
				<view v-if="detail.is_picture == 1" class="content-box">
					<view @click="yulan(detail.contentImage,index)" class="ww100"
						v-for="(item, index) in detail.contentImage" :key="index">
		...
		<view class="btns-wrap">
			...
			<template v-else>
				<button v-if="!is_virtual && !ispresale&& !detail.custom_form" class="add-cart"
	...
	export default {
		...
		onShareAppMessage() {
			...
			return {
				...
				imageUrl:self.detail.image?self.detail.image[0].file_path : ''
			};
		},
		methods: {
			scrollFunc(e) {
