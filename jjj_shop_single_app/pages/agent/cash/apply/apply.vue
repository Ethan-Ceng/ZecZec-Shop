		...
		<view class="form-wrap f30">
			<form @submit="formSubmit" @reset="formReset">
				<view class="p-0-20 pt30 txje">
					...
					<text
						class="f26 gray9 ml30">{{ words.cash_apply.words.min_money.value+settlement.min_money+'元' }}</text>
				</view>
				<view class="p-0-20 ">
					<view class="withd-bc">
							...
							<view class="p-30-0 border-b">
								<view class="d-b-c" :class="withdraw_type==10?'active':''" @click="TabType(10)">
									<view class="d-s-c flex-1">
										<image style="width: 28rpx;height: 28rpx;margin-right: 22rpx;"
											src="/static/wx.png" mode=""></image>
								...
								<template v-if="withdraw_type == 10">
									<view class="mt20">
										<input :disabled="hasRealName" class="p20 border" name="real_name"
											v-model="real_name" type="text" placeholder-class="grary"
											placeholder="请输入姓名" />
									</view>
									<view v-if="!hasRealName" class="gray9 f22 d-s-c">
										<text class="icon iconfont icon-gantanhao mr10"
						...
						<template v-if="hasType('20')">
							<view class="p-30-0 border-b">
								...
								<template v-if="withdraw_type==20">
									<view class="mt20">
										<input class="p20 border-tb" name="alipay_name" v-model="form.alipay_name"
											type="text" placeholder-class="grary" placeholder="请输入姓名" />
									</view>
									<view class="mt20">
										<input class="p20 border-tb" name="alipay_account" v-model="form.alipay_account"
											type="text" placeholder-class="grary" placeholder="请输入支付宝账号" />
									</view>
								</template>
							</view>
						</template>
						<template v-if="hasType('30')">
							<view class="p-30-0 border-b">
								<view class="d-b-c" :class="withdraw_type==30?'active':''" @click="TabType(30)">
									<view class="d-s-c flex-1">
										<image style="width: 28rpx;height: 22rpx;margin-right: 22rpx;"
											src="/static/yinxingqia.png" mode=""></image>
								...
								<template v-if="withdraw_type==30">
									<view class="mt20">
										<input class="p20 border-tb" name="bank_name" v-model="form.bank_name"
											type="text" placeholder-class="grary" placeholder="请输入银行名称" />
									</view>
									<view class="mt20">
										<input class="p20 border-tb" name="bank_account" v-model="form.bank_account"
											type="text" placeholder-class="grary" placeholder="请输入姓名" />
									</view>
									<view class="mt20">
										<input class="p20 border-tb" name="bank_card" v-model="form.bank_card"
											type="text" placeholder-class="grary" placeholder="请输入银行卡号" />
									</view>
								</template>
							</view>
						</template>
				...
				<view class="d-c-c mt60" style="border: 16rpx solid #F2F2F2">
						...
						<wx-open-subscribe :template="temlIds" id="subscribe-btn" @success="subscribeSuccess"
							...
							<div v-is="'script'" type="text/wxtag-template">
								<div class="subscribe-btn" :style="{
									...
									{{ words.cash_apply.words.submit.value }}
								</div>
							</div>
						</wx-open-subscribe>
					</template>
					<template v-else>
						<button type="primary" class="btn-red flex-1"
							@click="formSubmit">{{ words.cash_apply.words.submit.value }}</button>
					</template>
					<!-- #endif -->
					<!-- #ifndef H5 -->
					<button type="primary" class="btn-red flex-1"
