				...
				<view class="p-0-32">
					<template v-if="withdraw_type == 20">
						...
						<view class="cash-input-item">
							...
							<input class="p20 flex-1" name="alipay_name" type="text" placeholder-class="grary"
								placeholder="请输入姓名" />
						</view>
						<view class="cash-input-item mb48">
							<view class="input-label">账号</view>
							<input class="p20 flex-1" name="alipay_account" type="text" placeholder-class="grary"
					...
					<template v-if="withdraw_type == 30">
						...
						<view class="cash-input-item">
							...
							<input class="p20 flex-1" name="bank_account" type="text" placeholder-class="grary"
								placeholder="请输入姓名" />
						</view>
						<view class="cash-input-item">
							<view class="input-label">账号</view>
							<input class="p20 flex-1" name="bank_card" type="text" placeholder-class="grary"
								placeholder="请输入银行卡号" />
						</view>
						<view class="cash-input-item mb48">
							<view class="input-label">开户行</view>
							<input class="p20 flex-1" name="bank_name" type="text" placeholder-class="grary"
					...
					<view class="mb48">
						<view class="withd-bc">
							...
							<view class="withd-bcb f26" v-if="money * 1 > balance * 1"><text
									class="red">输入金额超过可提现金额</text></view>
						...
						<view class="mb48" v-if="withdraw_type == 10">
							...
							<view class="cash-input-item mb20">
								<input :disabled="hasRealName" :class="{ disabled: hasRealName }"
									class="p20 flex-1 userInput" v-model="real_name" name="real_name" type="text"
									placeholder-class="grary" placeholder="请输入姓名" />
							</view>
					...
					<view class="d-c-c" style="border: 16rpx solid #ffffff">
						<button v-if="money * 1 <= balance * 1" class="btn-red flex-1" @click="popup">提交申请</button>
			...
			getData() {
				...
				self._get(
					...
					function(res) {
						...
						self.payType = res.data.pay_type;
						self.withdraw_type = self.payType[0];
						self.pop_type = self.withdraw_type;
						self.loadding = false;
						uni.hideLoading();
					}
				);
