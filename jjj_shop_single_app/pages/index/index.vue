	...
	<view :data-theme="theme()" :class="theme() || ''">
		<view class="top_head pr" :style="'background-color: '+bgcolor+';'">
			...
			<view class="d-b-c" :style="topBarHeight() == 0 ? '': 'height:'+topBarHeight()+'px;'">
				...
				<view v-if="hide_search" class="flex-1"></view>
				<template v-else>
					<!-- #ifndef MP-WEIXIN -->
					<view class="d-s-c flex-1">
						<view class="top_search special" style="color:#999;" @click="showSearch=true">
							<text class="icon iconfont icon-sousuo" :style="searchIconTxt"></text>搜索商品
						</view>
						<view class="top_search_right" @click="scanQrcode" :style="rightSearch"><text
								class="icon iconfont icon-saoyisao1"></text></view>
					</view>
					<!-- #endif -->
					<!-- #ifdef MP-WEIXIN -->
					<view class="d-s-c flex-1">
						<view class="top_search search-wx" :style="'height:'+topBarHeight()+'px;color:#999;'"
							@click="showSearch=true">
							<text class="icon iconfont icon-sousuo"></text>搜索商品
						</view>
						<view class="top_search_right"
							:style="'height:'+topBarHeight()+'px;margin-right: '+topBarRight()+';'" @click="scanQrcode">
			...
			getData() {
				...
				self._get('index/index', {
					...
					function(res) {
						...
						self.hide_search = res.data.page.style.hide_search;
...
<style lang="scss">
	...
	.follow-gzh {
		...
		left: 0;
		right: 0;
		bottom: calc($footer-tabBar-height + env(safe-area-inset-bottom));
		border-radius: 16rpx;
