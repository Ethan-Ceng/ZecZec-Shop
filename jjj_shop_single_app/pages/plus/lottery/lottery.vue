<template>
	<view class="bg-lottery pr" :style="'background-image: url(' + bg_image + ');'">
		...
		<view class="ww100 mb23 mt40">
			<button type="default" class="record" @click="gotoList('/pages/plus/lottery/WinningRecord')">
				中奖记录
				<text class="icon iconfont icon-bofang"></text>
			</button>
		</view>
		<view class="content" v-if="listData.length>0">
			<swiper class="swiper" vertical circular autoplay interval="2000"
				:display-multiple-items="listData.length>=5?5:listData.length">
				<swiper-item v-for="(item, index) in listData" :key="index">
					<view class="box">
						<view class="text-ellipsis">{{ points_name(item.showText) }}</view>
					</view>
				</swiper-item>
			</swiper>
		</view>
