<template>
	<view :class="Visible ? 'product-popup open' : 'product-popup close'" @touchmove.stop.prevent="" @click="closePopup">
		...
		<view class="main" v-on:click.stop>
			...
			<view class="btns"><button type="primary" class="confirm-btn" @click="closePopup()">选好了</button></view>
		</view>
	</view>
</template>
				...
				this.form.specData.spec_attr.forEach((item, index) => {
					item.spec_items.forEach((citem, cindex) => {
						if (!citem.checked) {
							...
							// console.log(citem);
							// citem.checked = false;
							this.form.specData.spec_attr[index].spec_items[cindex].checked = false;
						} else {
							// citem.checked = true;
							this.form.specData.spec_attr[index].spec_items[cindex].checked = false;
	...
	methods: {
		...
		selectAttr(attr_index, item_index) {
			...
			if (curItem.checked) {
				...
				self.form.specData.spec_attr[attr_index].spec_items[item_index].checked = false;
				self.form.productSpecArr[attr_index] = null;
			} else {
				for (let i = 0; i < items.length; i++) {
					self.form.specData.spec_attr[attr_index].spec_items[i].checked = false;
				}
				self.form.specData.spec_attr[attr_index].spec_items[item_index].checked = true;
