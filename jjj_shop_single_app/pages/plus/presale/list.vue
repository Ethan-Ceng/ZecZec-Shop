		...
		<view class="presale-list-wrap" v-if="!loading">
			<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'"
				...
				<view class="list p-0-20">
					<view class="item d-stretch" v-for="(item, index) in listData" :key="index" @click="gotoDetail(item.product.product_id)">
							...
							<view class="slider-box ww100 d-b-c">
								<view>
									<view class="f32 redF11"><text
											class="f16 fb">￥</text>{{ (item.sku[0].product_price*1 - item.sku[0].advance_price*1 + item.money*1).toFixed(2) }}
									</view>
									<view class="f20 gray9">预估到手价</view>
								</view>
								<view class="right-btn ml30"><button type="primary">抢购</button></view>
							</view>
						</view>
					</view>
				</view>
