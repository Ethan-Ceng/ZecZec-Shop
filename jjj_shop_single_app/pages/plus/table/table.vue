	...
	<view  :data-theme='theme()' :class="theme() || ''"  v-if="!loading">
		<view v-for="(item,index) in tableData" :key='index'>
					...
					<view class="uni-list">
						<checkbox-group @change="changeRadioGroup($event,index)">
							<label class="d-s-c make-item mb20" v-for="(ritem,rindex) in getName(item.select_value)" :key='ritem'>
								<view>
									<checkbox style="transform:scale(0.7)" color="#EE1413" :checked="getRadioGroup(index,ritem) != -1 " :value="ritem" />
								</view>
								<view class="f26 color-57">{{ritem}}</view>
							</label>
						</checkbox-group>
			...
			<template v-if="item.type == 'text'">
				...
				<view class="table-item">
					<view class="d-s-c make-top">
						<view class="fb color-28 f26">
							<text :class="item.is_required != 'false' ? 'red':'white'">
							*
							</text>
						{{item.name}}
					</view>
			...
			<template v-if="item.type == 'date'">
				...
				<view class="table-item">
					<view class="d-s-c make-top">
						<view class="fb color-28 f26">
							<text :class="item.is_required!='false'?'red':'white'">*</text>{{item.name}}</view>
					</view>
					<view class="uni-list">
			...
			<template v-if="item.type == 'area'">
				...
				<view class="table-item">
					...
					<view class="uni-list make-item">
						<view class="input-box flex-1 selectpicker mb20"  @click.stop="changeArea(index)">
							<input type="text" placeholder-class="grary9" placeholder="请选择地区" v-model="item.value" :disabled="true"/>
						</view>
			...
			<template v-if="item.type == 'image'">
				...
				<view class="table-item">
					...
					<view class="uni-list">
						<button type="default" class="btn-red mb20 theme-btn" @click="openUpload(index)">上传图片</button>
		...
		<view class="ww100">
			<view class="submit theme-btn" @click="submit">提交</view>
		</view>

		<mpvue-city-picker v-if="is_load" ref="mpvueCityPickerRef" :province="province" :city="city" :area="area"
	...
	export default {
		...
		methods: {
			...
			changeRadioGroup(e, index) {
				...
				this.tableData[index].value = arr.join(",");
			...
			changeArea(index) {
				...
				this.$refs.mpvueCityPickerRef.show();
			},
			/*确定选择的省市区*/
			onConfirm(e) {
...
<style lang="scss">
	...
	.radio_group.active {
		background-color: #ffffff;
		border: 11rpx solid $dominant-color;
	}
