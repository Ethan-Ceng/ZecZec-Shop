	...
	<view class="fight-group-detail" v-if="!loadding">
		...
		<view class="fight-users mt20 bg-white">
			...
			<view class="user-list d-c-c">
				...
				<view class="user-box user-who d-c-c" v-if="dif_people > 0">
					?
				</view>
			</view>
			<view class="d-c-c mt30 gray3" v-if="dif_people > 0">
				还差<text class="fb">{{dif_people}}</text>人成团
			</view>
			<view class="d-c-c mt30 gray3" v-else>
				拼团已完成
			</view>
			...
			getData() {
				...
				self._get(
					...
					function(res) {
						...
						self.dif_people = self.productData.assemble_num - self.billData.billUser.length < 0 ? 0 : self
							.productData.assemble_num -
