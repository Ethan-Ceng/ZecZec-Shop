						...
						<template v-if="item.order_status.value != 21">
							<view class="d-e-c">
								...
								<view class="d-e-c mt10"
									...
									<button v-if="item.pay_type.value ==20 && item.pay_source =='wx' && is_send_wx"
										@click="wxOrder(item)" class="pay-btn theme-btn wxorder">确认收货</button>
									<button v-else @click="orderReceipt(item.order_id)"
					...
					<view class="order-bts" v-else>
						<block v-if="item.order_status.value ==10">
							...
							<block v-if="item.delivery_status.value == 20 && item.receipt_status.value == 10">
								<button class="theme-btn wxorder"
									v-if="item.pay_type.value ==20 && item.pay_source =='wx' && is_send_wx"
