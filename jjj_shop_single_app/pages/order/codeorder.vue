			...
			<view class="order-list">
				<view class="item" v-for="(item, index) in listData" :key="index">
					<view class="order-head d-b-c">
						...
						<view class="state">
							<text class="red" v-if="item.delivery_status == 0">待发货</text>
							<text class="red" v-if="item.delivery_status == 1">已发货</text>
						</view>
					</view>
					<!--一个商品显示-->
					<view class="one-product d-s-c" @click="gotoOrder(item.order_id)">
						...
						<view class="pro-info flex-1">{{ item.product_name }}</view>
						<view class="total-count">
							<view class="left-shadow"></view>
							<view class="price theme-price f22">
								¥
								<text class="f40 theme-price">{{ item.product_price }}</text>
							</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			/*跳转页面*/
			gotoOrder(e) {
				this.gotoPage('/pages/order/code-order-detail?order_id=' + e);
			},
