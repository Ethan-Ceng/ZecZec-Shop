		...
		<div class="common-seach-wrap">
			<el-form size="small" :inline="true" :model="formInline" class="demo-form-inline">
				...
				<el-form-item label="关键词">
					<el-input placeholder="请输入微信昵称|手机号|ID" v-model="formInline.nickName"></el-input>
				</el-form-item>
				<el-form-item>
		...
		<div class="product-content">
            ...
            <template #default="scope">
              <el-button type="primary" size="small" @click="selectUser(scope.row)">选择</el-button>
					...
					<el-table-column type="selection" :selectable="selectableFunc" width="45"></el-table-column>
				</el-table>
			</div>
		...
		setup(props) {
			...
			const getTableList = () => {
				...
				DataApi.getUser(params, true).then(data => {
						state.loading = false;
						/*判断是否需要去重*/
						if (props.excludeIds && typeof(props.excludeIds) != 'undefined' && props.excludeIds
							.length > 0) {
							const list = props.excludeIds.map(obj => obj.user_id);
							data.data.list.data.forEach(item => {
								if (list.indexOf(item.user_id) > -1) {
									item.noChoose = false;
								} else {
									item.noChoose = true;
								}
