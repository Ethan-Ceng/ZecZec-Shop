	...
	<div class="upload-wrap">
		...
		>
			...
			<div class="fileContainer">
				<div class="file-type">
					<ul>
						<li :class="activeType == item.category_id ? 'item active' : 'item'" v-for="(item, index) in typeList" :key="index" @click="selectTypeFunc(item.category_id)">
							{{ item.name }}
						</li>
					</ul>
				</div>
...
export default {
	...
	methods: {
		...
		confirmFunc() {
			...
			this.$emit('closeSystem', list);
		},

		/*取消*/
		cancelFunc() {
			this.$emit('closeSystem', null);
		}
	}
};
</script>
