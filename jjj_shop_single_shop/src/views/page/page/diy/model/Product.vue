<template>
	...
	<div class="drag optional" :class="{ selected: index === selectedIndex }"
		@click.stop="$parent.$parent.onEditer(index)" :style="{
					...
					<li class="product-item" v-for="(product, index) in item.data" :key="index">
						...
						<template v-if="item.style.column == 1">
							<div class="product-item-box">
								...
								<div class="product-info">
									<!-- 商品名称 -->
									<div v-if="item.params.productName" :style="{color: item.style.product_name_color}"
										class="product-title">
									...
									<div class="price d-s-c">
										<div v-if="item.params.productPrice"
											:style="{color: item.style.product_price_color}">
											<span>¥</span>
											<span class="f18 fb">{{ product.product_price }}</span>
										</div>
										<div class="ml10 gray9 text-d-line"
											v-if="item.params.linePrice && product.line_price > 0"
											:style="{color: item.style.line_price_color}">
											<span>¥</span>
						...
						<template v-else>
							...
							<div class="product-info p-0-10">
								<div v-if="item.params.productName" class="product-title"
									:style="{color: item.style.product_name_color}">{{ product.product_name }}</div>
								<div class="price d-s-c f12">
									...
									<div class="ml4 gray9 text-d-line"
										v-if="item.params.linePrice && product.line_price > 0"
										:style="{color: item.style.line_price_color}">¥{{ product.line_price }}</div>
								</div>
							</div>
						</template>
					</li>
