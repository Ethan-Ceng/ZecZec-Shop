		...
		<div class="product-content">
			<div class="table-wrap">
				<el-tabs v-model="activeName" @tab-change="handleClick">
					...
					<el-tab-pane :label="'待取消(' + order_count.cancel + ')'" name="cancel"></el-tab-pane>
					<el-tab-pane :label="'已取消(' + order_count.canceled + ')'" name="canceled"></el-tab-pane>
				...
				<el-table size="small" :data="tableData.data" :span-method="arraySpanMethod" border style="width: 100%"
								...
								<div class="product-info" v-for="(item, index) in scope.row.product" :key="index">
									...
									<div class="info">
										<div class="name gray3 product-name">
											<span>
												{{ item.product_name }}
												<span v-if="item.is_gift==1" class="red">赠品</span>
											</span>
										</div>
					...
					<el-table-column prop="pay_price" label="实付款">
						<template #default="scope">
							<div v-if="!scope.row.is_top_row">
								...
								<p class="orange" v-if="scope.row.order_source==80">定金：{{ scope.row.advance.pay_price }}
								</p>
					...
					<el-table-column prop="pay_type.text" label="支付方式">
						<template #default="scope">
							<div v-if="!scope.row.is_top_row">
								...
								<div v-else>
									<div class="gray9">{{ scope.row.pay_type.text }}</div>
								</div>
							</div>
						</template>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作" width="210">
						...
						<template #default="scope">
							<div v-if="!scope.row.is_top_row">
								...
								<el-button
									v-if="scope.row.delivery_type.value==10&&scope.row.delivery_status.value==20&&scope.row.is_single == 0"
									@click="getLogistics(scope.row)" link type="primary" size="small"
									v-auth="'/order/order/express'">查看物流
								</el-button>
								<el-button
									v-if="scope.row.pay_status.value == 20 && scope.row.delivery_status.value == 10
                  ...
                  && scope.row.delivery_type.value == 30" @click="addClick(scope.row)" type="text" size="small"
								...
								<el-button
									v-if="scope.row.pay_status.value == 20 && scope.row.delivery_status.value == 30 && scope.row.order_status.value != 20 && scope.row.order_status.value != 21 && scope.row.is_single == 1"
									@click="addClick(scope.row)" type="text" size="small"
									v-auth="'/order/order/delivery'">
									继续发货
								</el-button>
                  ...
                  && scope.row.order_status.value == 10" @click="cancelClick(scope.row)" type="text" size="small"
								...
								<el-button
									v-if="scope.row.pay_type.value == 20 &&scope.row.pay_status.value == 20 && scope.row.delivery_status.value == 20 && scope.row.wx_delivery_status==10 && scope.row.pay_source=='wx' && is_send_wx==true"
