		...
		<el-form-item label="规格明细：" v-if="form.model.spec_many && form.model.spec_many.spec_list.length > 0">
			<div>
				...
				<el-input size="small" v-model="batchData.line_price" placeholder="划线价"
					style="width: 160px;padding-left: 4px;"></el-input>
				<el-input size="small" v-model="batchData.low_price" placeholder="成本价"
					style="width: 160px;padding-left: 4px;"></el-input>
			...
			<el-table size="small" :data="form.model.spec_many.spec_list" :span-method="objectSpanMethod" border
				...
				v-if="form.model && form.model.spec_many && form.model.spec_many.spec_attr">
				...
				<el-table-column label="详情" width="240">
					<template #default="scope">
						<el-form-item label="" :rules="[{ required: true, message: ' ' }]" style="margin-bottom: 0;"
							:prop="'model.spec_many.spec_list.' + scope.$index + '.spec_form.detail'">
							<el-input prop="detail" v-model="scope.row.spec_form.detail" autosize type="textarea" />
						</el-form-item>
					</template>
				</el-table-column>
				...
				<el-table-column label="类型">
					<template #default="scope">
						<el-form-item label="" :rules="[{ required: true, message: '' }]" style="margin-bottom: 0;"
							:prop="'model.spec_many.spec_list.' + scope.$index + '.spec_form.type'">
							<el-radio-group v-model="scope.row.spec_form.type" size="lager">
								<el-radio :label="0">主商品</el-radio>
								<el-radio :label="1">推荐加购</el-radio>
							</el-radio-group>
						</el-form-item>
					</template>
				</el-table-column>
				...
				<el-table-column label="成本价">
					<template #default="scope">
						<el-form-item label="" style="margin-bottom: 0;"><el-input size="small" prop="low_price"
								v-model="scope.row.spec_form.low_price"></el-input></el-form-item>
					</template>
				</el-table-column>
