  ...
  <div>
    ...
    <el-form-item label="商品管理" prop="tableData" :rules="rulesProduct">
      ...
      <div class="d-s-c pt16 max-w460 active-all-set">
        ...
        <div class="flex-1">
          <el-input v-model="all_assemble_price" type="number" min="0" placeholder="拼团价"></el-input>
        </div>
        <div class="flex-1 ml10">
          <el-input v-model="all_assemble_stock" type="number" min="0" placeholder="拼团库存"></el-input>
        </div>
        <div class="flex-1 ml10">
          <el-input v-model="all_assemble_num" type="number" min="0" placeholder="开团人数"></el-input>
        </div>
        <div class="flex-1 ml10">
      ...
      <div class="pt16">
        <el-table size="small" :data="form.tableData" :span-method="objectSpanMethod" border style="width: 100%;">
          ...
          <el-table-column label="规格" width="180">
            <template #default="scope">
              ...
              <div v-if="scope.row.spec_type == 20">
                ...
                <el-button v-else size="small" icon="Plus" @click="specsFunc(scope.$index, scope.row)">添加规格
                </el-button>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="拼团价">
            <template #default="scope">
              <el-input size="small" :class="{ isvalid: scope.row.priceValid == true }" type="number" min="0"
                step="0.01" v-model="scope.row.assemble_price"></el-input>
            </template>
          </el-table-column>
          <el-table-column label="拼团库存">
            <template #default="scope">
              <el-input size="small" :class="{ isvalid: scope.row.stockValid == true }" type="number" min="0"
                v-model="scope.row.assemble_stock"></el-input>
            </template>
          </el-table-column>
          <el-table-column prop="sales_num" label="已售"></el-table-column>
          <el-table-column label="开团人数">
            <template #default="scope">
              <el-input size="small" :class="{ isvalid: scope.row.assemble_numValid == true }" type="number" min="2"
                v-model="scope.row.assemble_num"></el-input>
            </template>
          </el-table-column>
          <el-table-column label="限购数量">
            <template #default="scope">
              <el-input size="small" :class="{ isvalid: scope.row.limit_numValid == true }" type="number" min="1"
                v-model="scope.row.limit_num"></el-input>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" :width="type == 'edit' ? 140 : 60">
            <template #default="scope">
              <el-button v-if="type == 'edit'" @click="showFunc(scope.$index, scope.row)" type="text" size="small">
                {{ scope.row.is_delete == 0 ? '隐藏' : '显示' }}</el-button>
          ...
          for (let i = 0; i < this.form.tableData.length; i++) {
            ...
            if (!flag) {
              let reg = item.assemble_price == '' || item.assemble_stock == '' || item.limit_num == '' || item.sort ==
                '';
              let reg2 = parseFloat(item.assemble_price) == 0 || parseInt(item.assemble_stock) < 1 || parseInt(item
                .limit_num) < 1 || parseInt(item.assemble_num) < 1 || (item.spec_type == 20 && !item.spec_name);
              if (reg || reg2) {
                flag = true;
              }
            }
            ...
            if (parseInt(item.assemble_num) > 1) {
              item.assemble_numValid = true;
            } else {
              item.assemble_numValid = false;
            }
    ...
    methods: {
      ...
      allApply() {
        this.form.tableData.forEach((v)=>{
          if(parseInt(this.all_assemble_price) && parseInt(this.all_assemble_price) > 0){
            v.assemble_price = this.all_assemble_price;
          }
          if(parseInt(this.all_assemble_stock) && parseInt(this.all_assemble_stock) > 0){
            v.assemble_stock = this.all_assemble_stock;
          }
          if(parseInt(this.all_assemble_num) && parseInt(this.all_assemble_num) > 0){
            v.assemble_num = this.all_assemble_num;
          }
