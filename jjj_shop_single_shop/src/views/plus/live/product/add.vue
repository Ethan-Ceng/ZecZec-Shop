<template>
	...
	<el-dialog title="添加商品" :modal-append-to-body="false" :before-close="closeFunc" v-model="dialogVisible"
		...
		<!--商品选择-->
		<Product :isproduct="isproduct" :excludeIds="exclude_ids" :islist="false" :product_type='product_type'
...
<script>
	...
	export default {
		...
		methods: {
			...
			getData() {
				LiveApi.toEditProduct({
					live_product_id: this.productId
				}).then(res => {
					this.ruleForm = res.data;
					this.product_img = res.data.product.product_image;
				});
			},
			...
			closeProductFunc(e) {
				if (e.type == 'success') {
					this.ruleForm.product_id = e.params.product_id;
					this.product_img = e.params.product_image;
					this.productName = e.params.product_name;
					this.ruleForm.name = e.params.product_name;
					this.ruleForm.shop_supplier_id = e.params.shop_supplier_id;
					this.ruleForm.cover_img = e.params.product_image;
				}
