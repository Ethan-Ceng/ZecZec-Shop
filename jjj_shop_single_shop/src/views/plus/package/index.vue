<template>
	<div class="user">
			...
			<el-form :inline="true" :model="formInline" class="demo-form-inline">
				...
				<el-form-item>
					...
					<el-button type="primary" @click="addClick" icon="Plus"
						v-auth="'/plus/package/add'">添加活动</el-button>
				</el-form-item>
			</el-form>
		</div>
			...
			<el-form ref="form" :model="form" label-width="100px">
				<div class="table-wrap">
					<el-table :data="tableData" border style="width: 100%" v-loading="loading">
						<el-table-column prop="gift_package_id" label="ID"></el-table-column>
						...
						<el-table-column prop="total_num" width="80" label="发放数量"></el-table-column>
						<el-table-column width="80" label="券码类型">
							<template #default="scope">
								<label v-if="scope.row.code_type == 10">一批一码</label>
								<label v-if="scope.row.code_type == 20">一物一码</label>
							</template>
						</el-table-column>
						<el-table-column prop="people" width="80" label="参与人数"></el-table-column>
						<el-table-column prop="status.text" width="80" label="状态"></el-table-column>
						<el-table-column fixed="right" width="280" label="操作">
							<template #default="scope">
								...
								<el-button v-if="scope.row.status.value==1" v-auth="'/plus/package/send'"
									@click="sendClick(scope.row.gift_package_id)" type="text" size="small"> 发布
								</el-button>
								<el-button v-else="" v-auth="'/plus/package/send'"
									@click="endClick(scope.row.gift_package_id)" type="text" size="small"> 终止
								</el-button>
								<el-button v-auth="'/plus/package/orderlist'"
									@click="orderClick(scope.row.gift_package_id)" type="text" size="small"> 购买记录
								</el-button>
								<el-button v-auth="'/plus/package/qrcode'" @click="qrcodeClick(scope.row)" type="text"
									size="small"> 推广二维码
								</el-button>
								<el-button v-auth="'/plus/package/delete'"
									@click="deleteClick(scope.row.gift_package_id)" type="text" size="small"> 删除
								</el-button>
							</template>
						</el-table-column>
					</el-table>
		...
		<!--会员等级-->
		<Qrcode v-if="open_qrcode" :open_qrcode="open_qrcode" :form="currentModel"
			@closeDialog="closeDialogFunc($event, 'qrcode')"></Qrcode>
	</div>
</template>
<script>
