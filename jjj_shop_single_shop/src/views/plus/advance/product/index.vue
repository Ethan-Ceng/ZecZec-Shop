<template>
  ...
  <div class="user" v-loading="loading">
    <div class="common-form d-s-c">
      <span>活动商品</span>
      <div class="ml20"><el-button type="primary" size="small" icon="Plus" @click="changeProduct" v-auth="'/plus/advance/product/add'">选择商品</el-button></div>
    </div>
      ...
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%">
          ...
          <el-table-column label="尾款">
            <template #default="scope">
              <span class="orange fb">{{ (scope.row.sku[0].product_price-scope.row.sku[0].advance_price).toFixed(2) }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="110">
            <template #default="scope">
              <el-button v-auth="'/plus/advance/product/edit'" @click="editClick(scope.row.advance_product_id)" type="text" size="small">编辑</el-button>
              <el-button v-auth="'/plus/advance/product/delete'" @click="deleteClick(scope.row.advance_product_id)" type="text" size="small">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
.advance-list .el-input-number--small {
  width: auto;
}
</style>
