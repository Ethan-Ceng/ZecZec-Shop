...
const formState = reactive<RuleForm>({
	...
	consented: false, // 是否同意
	pay_type: 10, // 支付方式，默認餘額支付
	payPrice: 0, // 下單金額，由用戶天下餓
	confirmStatus: false, // 是否可以下單
	confirmError: "",
});
...
const fetchDetail = async (productId) => {
	...
	try {
		...
		if (data && data.detail) {
			...
			productRecommend.value = product_sku.filter(
				(item) => item.type - 0 !== 0
			);
			productSku.value = product_sku.find(
				(item) => item.product_sku_id - 0 === productSkuId.value - 0
			);
	...
	try {
		...
		if (data && data.regionData) {
			...
			if (userDetail.value.addressDefault) {
				formState.province_id = get(
					userDetail.value,
					"addressDefault.province_id"
				);
				formState.city_id = get(
					userDetail.value,
					"addressDefault.city_id"
				);
				formState.region_id = get(
					userDetail.value,
					"addressDefault.region_id"
				);
				formState.detail = get(
					userDetail.value,
...
const cityList = computed(() => {
	const provinceIndex = provinceList.value.findIndex(
...
const districtList = computed(() => {
	...
	if (provinceIndex !== -1) {
		...
		if (cityIndex !== -1) {
			const reginArray =
				get(regionList.value, `[2][${provinceIndex}][${cityIndex}]`) ||
				[];
			return reginArray;
		}
	}
...
onMounted(() => {
	...
	// 如果未登錄就跳轉到登錄頁面
    if (isEmpty(userDetail.value)) {
        // 跳轉到/login
        router.push({
            path: '/login',
            query: { redirect: route.fullPath }  // 可選：保存當前路徑，以便登錄後返回
        });
        return;  // 重要：阻止後續代碼執行
    }
...
const fetchOrder = async () => {
	setLoading(true);
	try {
		const { remark, payPrice, pay_type } = formState;
		let products = [
			{
				product_id: productId.value,
				product_sku_id: productSku.value.spec_sku_id,
...
const goPayOrder = async (order_id) => {
	setLoading(true);
	try {
	    const { remark, pay_price } = formState;
		const params = {
			order_id,
			payType: 10,
			pay_source: "h5",
...
const handlePayPriceChange = () => {
    formState.confirmStatus = false;
    if (formState.payPrice >= productSku.value.min_price && formState.payPrice <= productSku.value.max_price && formState.consented) {
        formState.confirmStatus = true;
    }
}

const agreeOrOpposed = () => {
    formState.confirmStatus = false;
    if (formState.payPrice >= productSku.value.min_price && formState.payPrice <= productSku.value.max_price && formState.consented) {
        formState.confirmStatus = true;
    }
			...
			<div class="w-full flex">
				<div class="inline-block lg:float-left w-full lg:w-1/3">
					<div
						...
						:style="`background-image: url('${get(
              productDetail,
              'product_image',
              ''
            )}')`"
				...
				<div class="py-2 lg:px-2 px-4 flex-1">
					<a :href="`/project/${get(productDetail, 'product_id')}`">
						<h2
							class="text-base mb-0 inline-block mt-1 text-base font-bold"
					...
					<span class="font-bold">$ {{ get(productDetail, "total_money", "") }}</span>
					<span class="text-xs text-neutral-600">/ 目標 $ {{ get(productDetail, "total_money", "") }}</span>
					<p
			...
			<div class="w-full px-4 mb-8">
				...
				>
					<div class="flex mb-16 text-sm -mx-4">
						<div class="w-full px-4 lg:w-1/3">
							<router-link
								...
								:to="{ path: `/order/${productSku.product_id}` }"
							>更改回饋</router-link>
							<!-- sku資訊 -->
                  ...
                  <p class="w-full text-gray-500 font-normal text-xs">
                    預定售價
                    <span class="line-through">$ {{ get(productSku, 'line_price') }}</span>
                    ，現省 $ {{ get(productSku, 'line_price') - get(productSku, 'product_price') }}
                  </p>
									-->
								</div>
								<div class="text-xs my-2">
									...
									>剩餘 {{ get(productSku, "stock_num") }} 份</span>
								...
								<ul class="mt-4 pt-4 border-t text-gray-800 text-xs flex">
									<li class="mr-2 -indent-4 pl-4 leading-relaxed">
										...
										回饋率：{{ get(productSku, "commission_percent", 0) }}%
									</li>
									<li class="mr-2 -indent-4 pl-4 leading-relaxed">
										<el-icon size="14" color="#229f2a">
											<CircleCheckFilled />
										</el-icon>
										回饋週期：{{ get(productSku, "cycleString", "") }}
									</li>
								</ul>
							...
							<div class="flex items-center">
								<div class="whitespace-nowrap font-bold">
									<label class="mb-0 inline-block" for="order_price">購買金額</label>
								</div>
								<div class="whitespace-nowrap text-right flex-1">
									<input
										...
										v-model="formState.payPrice"
										@input="handlePayPriceChange"
									/>
								</div>
							</div>
                ...
                <div class="flex items-start text-sm py-2 border-t-1 border-gray-300">
                  ...
                  <div class="whitespace-nowrap text-right">
                    + $ {{ get(orderInfo, 'express_price', 0) }}
								...
								<!--                  </div>-->
								<!--                </div>-->
								<div class="flex items-start text-xl pb-2 pt-4 border-0 border-gray-300">
						...
						<div class="w-full px-4 lg:w-2/3">
							...
							<template v-if="productRecommend.length">
								<div v-show="showProductRecommend" class="js-option-details">
									<div class="mb-8 border-b border-neutral-200">
										...
										<el-checkbox-group v-model="specSkuIdList" @change="getOrderInfo">
											<div class="flex -mx-4 flex-nowrap overflow-auto pl-4 flex-stretch">
												<div
													v-for="productItem in productRecommend"
													:key="productItem.spec_sku_id"
												...
												>
													...
													<div class="p-4 border-2 border-inherit rounded text-neutral-200 mb-8 block">
														<div
															class="text-gray-600 font-bold mt-4 mb-2"
														>{{ get(productItem, "spec_item.spec_value") }}</div>
														<div class="text-black font-bold text-xl flex items-center">
															$ {{ get(productItem, "product_price") }}
															<span
																class="inline-block text-xs font-bold text-black bg-yellow-300 leading-relaxed px-2 ml-2 rounded-sm"
															>69 折</span>
															<p class="w-full text-gray-500 font-normal text-xs">
																預定售價
																<span class="line-through">$ {{ get(productItem, "line_price") }}</span>
																，現省 $
																{{
																get(productItem, "line_price") -
																get(productItem, "product_price")
																}}
															</p>
														</div>
														...
														<ul class="mt-4 pt-4 border-t text-gray-800 text-xs flex">
															...
															<li class="mr-2 -indent-4 pl-4 leading-relaxed">
																<el-icon size="14" color="#229f2a">
																	<CircleCheckFilled />
																</el-icon>可寄送至 香港、澳門、馬來西亞 等地區
															</li>
														</ul>
														<div class="text-center text-xs text-gray-600 pt-4 mt-4 border-t">
															<el-icon size="14">
																<Calendar />
															</el-icon>預計於 2025 年二月實現
														</div>
													</div>
												</div>
											</div>
									...
									<div class="js-variation-selection">
										<include-fragment
											class="block mb-8 js-variations-fragment"
											data-src="/order_variations/135367"
										></include-fragment>
									</div>
								</div>
								<!-- 付款方式-->
							...
							<div
								v-show="!showProductRecommend || productRecommend.length == 0"
								class="mt-8 js-payment-step"
							>
									...
									<el-radio-group class="w-full" v-model="formState.payment_method">
										...
										>
											<h3 class="ml-2 flex-1">
												<span class="block font-bold">餘額支付</span>
												<p class="flex text-xs text-gray-800 flex mt-1">
													<span>您的可用餘額</span>：
													<span>${{ get(userDetail, "balance", 0) }}</span>
												</p>
											</h3>
										</el-radio>
										<!--
                    ...
                    <el-radio
                      ...
                      <h3 class="ml-2 flex-1">
                        ...
                        <span class="mt-2 text-xs text-gray-800">需於指定時間內完成付款，超過時限則會取消交易</span>
								...
								<!-- 付款說明 start -->
								<div
									v-show="formState.payment_method === 'credit_card'"
								...
								<div
									v-show="formState.payment_method === 'atm'"
								...
								>
									<ul class="list-disc m-0 pl-4 text-slate-600">
										...
										<li>
											虛擬帳號轉帳因要等待銀行於工作天回傳，
											<b>限量的選項可能會在入帳前售罄並造成贊助失敗</b>。遇到例假日將會延後更新。若未來有遇到退款的情況，
											<b>退款金額會扣除轉帳交易所產生的手續費用</b>。臨櫃繳款的截止期限，必須在週一至週五（不含例假日）的下午三點半前完成，
											<b>週六的郵局轉帳將會造成交易失敗</b>。
										</li>
									</ul>
								</div>
								...
								<div class="p-4 js-zero-card-warning border hidden mt-2 rounded border-slate-200">
									<ul class="list-disc m-0 pl-4 text-slate-600">
										<li>您瞭解您的贊助是支援創意專案的一種方式，也瞭解創意實踐過程中充滿變數，專案不一定能確保回饋。</li>
										<li>您知悉且明瞭本募資專案屬預購性質，您與提案者之法律關係為買賣，如確定申請分期付款，不得以任何理由拒絕付款。</li>
										<li>
											zingala 銀角零卡為中租控股（股票代號
											5871）仲信資融提供之免信用卡分期之支付方式。
										</li>
										<li>
											透過 zingala
											銀角零卡分期系統審核方可使用，首次使用需備身份證件。
										</li>
										<li>
											相關資料僅供作 zingala
								...
								<div class="mb-2 mt-4">
									<el-form-item label="收件地點" prop="province_id">
										<el-select v-model="formState.province_id" placeholder="請選擇收件地點">
											<el-option
								...
								<div class="flex">
									<div class="mt-4 flex-auto">
										<el-form-item label="縣市" prop="city_id">
											<el-select v-model="formState.city_id" placeholder="請選擇縣市">
												<el-option
									...
									<div class="mt-4 flex-auto pl-4">
										<el-form-item label="鄉鎮市區" prop="region_id">
											<el-select v-model="formState.region_id" placeholder="請選擇鄉鎮市區">
												<el-option
								...
								<div class="mb-4 hidden">
									...
									<div class="js-store-info mr-4 inline-block" hidden>
										...
										<button
											class="button-text js-store-cancel pointer leading-none ml-1 px-2 text-zec-red text-xl"
											type="button"
										>×</button>
									</div>
									<details
										class="inline-block js-choose-store reset-details"
									...
									<input
										class="mb-4 w-full js-store-field"
										autocomplete="off"
										type="hidden"
										name="order[ship_to_store]"
								...
								<el-form-item label="聯絡電話" prop="phone">
									<el-input v-model="formState.phone" placeholder="請填寫真實手機號碼，以利取貨或聯絡收貨" maxlength="20"></el-input>
								</el-form-item>
								...
								<div class="p-4 rounded mb-4 bg-zinc-100 hidden">
									...
									<label class="font-bold mt-4">
										...
										>
											...
											<option value="大杯｜不鏽鋼原色">大杯｜不鏽鋼原色</option>
											<option value="大杯｜仙草黑">大杯｜仙草黑</option>
											<option value="大杯｜砂糖白">大杯｜砂糖白</option>
											<option value="大杯｜香草米">大杯｜香草米</option>
											<option value="大杯｜丹寧藍">大杯｜丹寧藍</option>
											<option value="大杯｜葡萄紫">大杯｜葡萄紫</option>
										</select>
									</label>
									<label class="font-bold mt-4">
										如有加購「肩揹帶」，請於此處選擇顏色
										...
										>
											<option value>選擇</option>
											<option value="肩揹帶｜芝麻">肩揹帶｜芝麻</option>
											<option value="肩揹帶｜沙灘">肩揹帶｜沙灘</option>
											<option value="肩揹帶｜深焙">肩揹帶｜深焙</option>
										</select>
									</label>
									<label class="font-bold mt-4">
										如有加購「杯套」，請於此處選擇顏色
										...
										>
											<option value>選擇</option>
											<option value="杯套｜芝麻">杯套｜芝麻</option>
											<option value="杯套｜沙灘">杯套｜沙灘</option>
											<option value="杯套｜深焙">杯套｜深焙</option>
										</select>
									</label>
									<label class="font-bold mt-4">
										如有加購「小包」，請於此處選擇顏色
										...
										>
											<option value>選擇</option>
											<option value="小包｜奶茶米">小包｜奶茶米</option>
											<option value="小包｜冬瓜綠">小包｜冬瓜綠</option>
											<option value="小包｜珍珠黑">小包｜珍珠黑</option>
										</select>
									</label>
									<label class="font-bold mt-4">
									...
									<label class="font-bold mt-4">
										...
										<input
											...
											name="order[survey_answers][100137]"
											type="text"
											value
										/>
									</label>
									<label class="font-bold mt-4">
										...
										<input
											class="w-full mb-0 mt-1 border border-gray-300 rounded px-2 focus:outline-none focus:ring-1 focus:ring-zec-blue focus:border-zec-blue"
											name="order[survey_answers][100138]"
											type="text"
											value
										/>
									</label>
									<label class="font-bold mt-4">
										...
										>
											<option value>選擇</option>
											<option value="杯袋｜芝麻">杯袋｜芝麻</option>
											<option value="杯袋｜沙灘">杯袋｜沙灘</option>
											<option value="杯袋｜深焙">杯袋｜深焙</option>
										</select>
									</label>
								</div>
								<!-- 訂單關聯問卷 end -->
								<div class="my-4 p-4 rounded bg-zinc-100" data-controller="invoice">
									...
									<el-form-item class="hidden" label="手機載具" prop="invoice_carrier">
										<el-input v-model="formState.invoice_carrier" placeholder="例：/AB1CD23" maxlength="20"></el-input>
									</el-form-item>

									<div class="flex" v-show="formState.invoice_type === 'commercial'">
										<el-form-item class="mr-4" label="發票統編" prop="invoice_ubn">
											<el-input v-model="formState.invoice_ubn" placeholder="例：12345678" maxlength="20"></el-input>
										</el-form-item>
										<el-form-item class="mr-4" label="發票抬頭" prop="invoice_heading">
											<el-input v-model="formState.invoice_heading" placeholder="例：12345678" maxlength="20"></el-input>
										</el-form-item>
									</div>
								</div>
								<!--  分期付款 start  -->
								...
								>
									<el-icon class="material-icons text-sm align-middle" size="14">
										<InfoFilled />
									</el-icon>以下資料僅供作 zingala
									銀角零卡分期服務驗證使用者身分，嘖嘖網站不會儲存亦不會提交給其他團體或個人。
									<label
										class="mt-4 font-bold"
										for="order_installment"
									>
										zingala 銀角零卡分期/ 分期期數
										<select
											...
											disabled="disabled"
											name="order[installment]"
											id="order_installment"
										>
											<option value>- 選擇分期期數 -</option>
										</select>
									</label>
									<label class="mt-4 font-bold" for="order_zero_card_buyer_name">
										zingala 銀角零卡分期/ 姓名
										<input
											placeholder="姓名"
											maxlength="10"
											...
											size="10"
											type="text"
											name="order[zero_card_buyer_name]"
											id="order_zero_card_buyer_name"
										/>
									</label>
									<label class="mt-4 font-bold" for="order_zero_card_buyer_phone">
										zingala 銀角零卡分期/ 電話
										<input
											placeholder="電話"
											maxlength="20"
											...
											size="20"
											type="text"
											name="order[zero_card_buyer_phone]"
											id="order_zero_card_buyer_phone"
										/>
									</label>
									<label class="mt-4 font-bold" for="order_zero_card_buyer_email">
										zingala 銀角零卡分期/ Email
										<input
											placeholder="Email"
											type="email"
											class="w-full border-red-200 rounded js-zero-card-input focus:outline-none focus:ring-1 focus:ring-zec-blue focus:border-zec-blue"
											name="order[zero_card_buyer_email]"
											id="order_zero_card_buyer_email"
										/>
									</label>
									<label class="mt-4 font-bold" for="order_zero_card_buyer_id">
										zingala 銀角零卡分期/ 身份證字號
										<input
											placeholder="身份證字號"
											minlength="10"
											maxlength="10"
											...
											size="10"
											type="text"
											name="order[zero_card_buyer_id]"
											id="order_zero_card_buyer_id"
										/>
									</label>
								</div>
								<!--  分期付款 end  -->
								<el-form-item class="mt-4" prop="consented">
									<el-checkbox v-model="formState.consented" @change="agreeOrOpposed">
										<span class="text-sm">
											我已閱讀並同意
											<router-link to="/agreement" target="_blank">服務條款</router-link>與
								...
								<el-button
									...
									:disabled="!formState.confirmStatus"
								...
								>
									...
									<span class="font-bold ml-2">
										$
										<span class="js-sum">{{ formState.payPrice }}</span>
									</span>
								</el-button>
