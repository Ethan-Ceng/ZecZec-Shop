...
const fetchDetail = async (orderId) => {
    ...
    try {
        ...
        if (data && data.order) {
            ...
            console.log("or", orderDetail.value.pay_status.value);

            // 时间转换 item.cycle
            orderDetail.value.cycleString = convertHoursToDH(
                orderDetail.value.product[0].productSku.cycle,
            );
        }
    } catch (err) {
        // console.warn(err)
...
<template>
    <div class="container px-4 lg:px-0 my-8">
        <div
            class="rounded bg-yellow-100 p-4 flex flex-wrap items-start mb-6 shadow text-yellow-800"
        >
            ...
            <p class="flex-1">
                ...
                ，需要修改贊助資料請
                <router-link
                    class="inline-block font-bold underline text-yellow-800"
                    :to="`/message/new?product_id=${orderDetail.product_id}`"
                    >聯絡提案團隊</router-link
                >。
                -->
            </p>
    ...
    <div class="container px-4 lg:px-0 mt-8 mb-16 cf">
        <div class="lg:float-left w-full lg:w-3/10">
            <div
                class="p-4 border-2 border-inherit rounded text-neutral-200 mb-8 block opacity-60"
            >
                <img
                    width="600"
                    height="200"
                ...
                <div class="maxh5 maxh-none-ns overflow-y-auto break-all">
                    ...
                    >
                        <div
                            v-html="get(orderDetail, 'product[0].detail')"
                        ></div>
                    </div>
                </div>
                <ul class="mt-4 pt-4 border-t text-gray-800 text-xs flex">
                    ...
                    <li class="mr-2 -indent-4 pl-4 leading-relaxed">
                        ...
                        回饋週期：{{ get(orderDetail, "cycleString", "") }}
                    </li>
                </ul>
                <!--
                <div
            ...
            <!--
            <router-link
                class="inline-block mb-2 text-zec-blue border-2 border-current rounded px-4 py-1 font-bold"
        ...
        <div class="lg:float-left w-full lg:pl-8 lg:w-7/10">
            <form
                class="p-4 rounded border-2 border-gray-200"
                id="edit_order_6281187"
                action=""
            ...
            >
                ...
                <div class="cf mb-4">
                    <a
                        class="font-bold text-black text-base hover:text-zec-blue"
                        :href="`/project/${get(orderDetail, 'product[0].product_id')}`"
                    >
                        {{ get(orderDetail, "product[0].product_name") }}
                    </a>
                </div>
                <label class="font-bold mb-2 mt-4" for="order_sum"
                    >總金額</label
                >
                $ {{ get(orderDetail, "pay_price") }}
                ...
                <div>
                    <label class="font-bold mb-2 mt-4" for="order_recipient"
                        >訂單生成時間</label
                    >
                    {{ get(orderDetail, "create_time") }}
                </div>
                <div v-if="get(orderDetail, 'pay_status.value') != 10">
                    <label class="font-bold mb-2 mt-4" for="order_recipient"
                        >訂單付款時間</label
                    >
                    {{ get(orderDetail, "pay_time", "") }}
                </div>
                <div class="js-details reset-details">
                    <!--
                    <button
                        class="bg-zinc-100 px-1 text-xs button-text js-summary"
                        type="button"
                    >
                        細節
                    </button>
                    ...
                    <div class="hidden p-4 text-sm bg-zinc-100 rounded mt-2">
                        <div class="flex items-start text-sm py-2 mt-4">
                            ...
                            <div class="whitespace-nowrap text-right">
                                $ {{ get(orderDetail, "order_price") }}
                            </div>
                        </div>
                        <div
                            class="flex items-start text-sm py-2 border-t-1 border-gray-300"
                        >
                            <div class="whitespace-nowrap font-bold flex-auto">
                                運費 ：{{
                                    get(orderDetail, "address.region.province")
                                }}
                            </div>
                            <div class="whitespace-nowrap text-right">
                                + $ {{ get(orderDetail, "express_price") }}
                            </div>
                        </div>
                        <div
                            class="flex items-start text-xl pb-2 pt-4 border-t-4 border-gray-300"
                        >
                            <div class="whitespace-nowrap font-bold flex-auto">
                                總價
                            </div>
                ...
                <div class="my-4 hidden">
                    <div class="p-4 rounded mb-4 bg-zinc-100">
                        <h3
                            class="border-b border-gray-200 pb-1 text-base mb-4 font-bold"
                        >
                            回饋調查
                        </h3>
                ...
                <!-- 回饋調查 end -->
                <div
                    class="my-4 p-4 rounded bg-zinc-100 hidden"
                ...
                >
                    ...
                    <label class="inline-block mr-4">
                        <input
                            data-invoice-target="personalSelector"
                            data-action="invoice#selectPersonal"
                            disabled="disabled"
                            type="radio"
                            value="personal"
                            checked="checked"
                            name="order[invoice_type]"
                    ...
                    <label class="inline-block">
                        <input
                            data-invoice-target="commercialSelector"
                            data-action="invoice#selectCommercial"
                            disabled="disabled"
                            type="radio"
                            value="commercial"
                            name="order[invoice_type]"
                            id="order_invoice_type_commercial"
                        />
                        <span class="font-bold">公司發票</span>
                    </label>
                    <fieldset class="" data-invoice-target="personalBlock">
                        <label
                            class="font-bold mb-1 mt-4"
                            for="order_invoice_carrier"
                            >手機載具</label
                        >
                        <input
                            pattern="\/[0-9A-Z.+-]{7}"
                            ...
                            data-invoice-target="carrier"
                            type="text"
                            value=""
                            name="order[invoice_carrier]"
                            id="order_invoice_carrier"
                        />
                    </fieldset>
                    <div
                        class="flex hidden"
                        data-invoice-target="commercialBlock"
                    >
                        <fieldset class="mr-4">
                            <label
                                class="font-bold mb-1 mt-4"
                                for="order_invoice_ubn"
                                >發票統編</label
                            >
                            <input
                                pattern="[0-9]{8}"
                                ...
                                class="w-64 rounded border border-gray-300 focus:outline-none focus:ring-1 focus:ring-zec-blue focus:border-zec-blue"
                                data-invoice-target="ubn"
                                required="required"
                                disabled=""
                                type="text"
                                name="order[invoice_ubn]"
                        ...
                        <fieldset>
                            <label
                                class="font-bold mb-1 mt-4"
                                for="order_invoice_heading"
                                >發票抬頭</label
                            >
                            <input
                                placeholder="組織名稱"
                                class="w-64 rounded border border-gray-300 focus:outline-none focus:ring-1 focus:ring-zec-blue focus:border-zec-blue"
                                data-invoice-target="heading"
                                disabled=""
                                type="text"
                                name="order[invoice_heading]"
                ...
                <label class="font-bold mb-2 mt-4" for="order_phone"
                    >聯絡電話</label
                >
                {{ get(orderDetail, "address.phone") }}
                <!--        <label class="font-bold mb-2 mt-4" for="order_postcode">郵遞區號</label>-->
                <label class="font-bold mb-2 mt-4" for="order_address"
                    >地址</label
                >
                {{ get(orderDetail, "address.region.province") }} -
                {{ get(orderDetail, "address.region.city") }} -
                {{ get(orderDetail, "address.region.region") }}
                <label class="mt-4 font-bold" for="order_country"
                    >收件地點</label
                >
