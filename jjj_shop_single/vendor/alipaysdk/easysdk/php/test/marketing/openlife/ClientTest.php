...
class ClientTest extends TestCase
{
    ...
    {
        $result = Factory::marketing()->openLife()->createImageTextContent("標題",
    ...
    public function testModifyImageTextContent()
    {
        ...
        if ($result->code == '10000') {
            ...
            $this->assertEquals('批次傳送訊息頻率超限', $result->subMsg);
        }
    ...
    {
        $result = Factory::marketing()->openLife()->sendText("測試");
        if ($result->code == '10000') {
            $this->assertEquals('10000', $result->code);
            $this->assertEquals('Success', $result->msg);
        } else {
    ...
    public function testSetIndustry()
    {
        ...
        if ($result->code == '10000') {
            ...
            $this->assertEquals('模板訊息行業一月只能修改一次', $result->subMsg);
        }
    }
    ...
    public function testGetIndustry()
    {
        ...
        $this->assertEquals('10000', $result->code);
        $this->assertEquals('Success', $result->msg);
        $this->assertEquals('IT科技/IT軟體與服務', $result->primaryCategory);
        $this->assertEquals('IT科技/IT軟體與服務', $result->secondaryCategory);
    }
}