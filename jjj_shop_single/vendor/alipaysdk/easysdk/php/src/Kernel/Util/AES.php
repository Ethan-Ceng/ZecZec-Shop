...
class AES
{
    ...
    public function aesEncrypt($plainText, $key)
    {
        try {
            if(strlen($key) == 0){
                throw new \Exception("AES加密失敗，plainText=".$plainText."，AES金鑰為空。");
            }
            //AES, 128 模式加密資料 CBC
            ...
            throw new \Exception("AES加密失敗，plainText=".$plainText."，keySize=".strlen($key)."。".$e->getMessage());
        }
    }
    ...
    public function aesDecrypt($cipherText, $key)
    {
        try{
            if(strlen($key) == 0){
                throw new \Exception("AES加密失敗，plainText=".$cipherText."，AES金鑰為空。");
            }
            //AES, 128 模式加密資料 CBC
            ...
            throw new \Exception("AES解密失敗，cipherText=".$cipherText."，keySize=".strlen($key)."。".$e->getMessage());
        }
    }
