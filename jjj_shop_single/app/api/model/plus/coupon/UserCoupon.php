...
class UserCoupon extends UserCouponModel
{
     ...
     * @param $days int 連續簽到天數
     * @param $sign_conf array 簽到配置
     * @return int
     */
     ...
     * 判斷優惠券是否已領取
     */
    public function getUserCouponStatus($user, $coupon_id)
    {
        if ($user) {
            return $this->where('user_id', '=', $user['user_id'])->where('coupon_id', '=', $coupon_id)->count();
        } else {
            return 0;
        }
