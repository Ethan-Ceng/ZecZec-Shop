 ...
 * @see baidu.dom.q
 *
 * @return {HTMLElement|null} 獲取的元素，查詢不到時返回null,如果引數不合法，直接返回引數.
 */
 ...
 * @param {Function} iterator 對每個陣列元素進行呼叫的函式，該函式有兩個引數，第一個為陣列元素，第二個為陣列索引值，function (item, index)。
 * @param {Object} [thisObject] 函式呼叫時的this指標，如果沒有此引數，預設是當前遍歷的陣列
 * @remark
...
/**
 * 對語言層面的封裝，包括型別判斷、模組擴充套件、繼承基類以及物件自定義事件的支援。
...
/**
 * 操作flash物件的方法，包括建立flash物件、獲取flash物件以及判斷flash外掛的版本號
 * @namespace baidu.swf
 */
baidu.swf = baidu.swf || {};
 ...
 * 
 * @param {Object} 	options 					建立flash的選項引數
 ...
 * @param {boolean} options.seamlesstabbing 	允許設定執行無縫跳格，從而使使用者能跳出flash應用程式。該引數只能在安裝Flash7及更高版本的Windows中使用。允許值：true/false
 * @param {boolean} options.devicefont 			設定靜態文字物件是否以裝置字型呈現。允許值：true/false
 * @param {boolean} options.swliveconnect 		第一次載入flash時瀏覽器是否應啟動Java。允許值：true/false
 * @param {Object} 	options.vars 				要傳遞給flash的引數，支援JSON或string型別。
 * 
 ...
 * @param {Object} 	options.vars 				要傳遞給flash的引數，支援JSON或string型別。
 * 
 * @param {HTMLElement|string} [container] 		flash物件的父容器元素，不傳遞該引數時在當前程式碼位置建立flash物件。
 * @meta standard
 ...
 * @see baidu.swf.create
 * @meta standard
 * @returns {HTMLElement} flash物件的例項
 */
...
baidu.flash._Base = (function(){
    ...
    return function(options, callBack){
         ...
         * @param {Array} params 傳入的引數組成的陣列,若不許要引數，需傳入空陣列
         * @param {Function} [callBack] 非同步呼叫後將返回值作為引數的呼叫回撥函式，如無返回值，可以不傳入此引數
         * @return {Null}
        */
 ...
 * @param {Object} createOptions 建立flash時需要的引數，請參照baidu.swf.create文件
 * @config {Object} vars 建立imageUploader時所需要的引數
 * @config {Number} vars.gridWidth 每一個預覽圖片所佔的寬度，應該為flash寛的整除
 * @config {Number} vars.gridHeight 每一個預覽圖片所佔的高度，應該為flash高的整除
 ...
 * @param {String} listBacgroundkUrl 佈局控制元件背景
 ...
 * @config {Object} createOptions 建立flash時需要的引數，請參照baidu.swf.create文件
...
baidu.flash.fileUploader = baidu.flash.fileUploader || function(options){
     ...
     * @param {Object} postData 鍵值對，上傳的POST資料
...
/**
 * 透過script標籤載入資料，載入完成由瀏覽器端觸發回撥
 * @name baidu.sio.callByBrowser
 * @function
 * @grammar baidu.sio.callByBrowser(url, opt_callback, opt_options)
 * @param {string} url 載入資料的url
 * @param {Function|string} opt_callback 資料載入結束時呼叫的函式或函式名
 * @param {Object} opt_options 其他可選項
 * @config {String} [charset] script的字元集
 * @config {Integer} [timeOut] 超時時間，超過這個時間將不再響應本請求，並觸發onfailure函式
 * @config {Function} [onfailure] timeOut設定後才生效，到達超時時間時觸發本函式
 * @remark
 * 1、與callByServer不同，callback引數只支援Function型別，不支援string。
 * 2、如果請求了一個不存在的頁面，callback函式在IE/opera下也會被呼叫，因此使用者需要在onsuccess函式中判斷資料是否正確載入。
 * @meta standard
...
/**
 * 透過script標籤載入資料，載入完成由伺服器端觸發回撥
 * @name baidu.sio.callByServer
 * @function
 * @grammar baidu.sio.callByServer(url, callback[, opt_options])
 * @param {string} url 載入資料的url.
 * @param {Function|string} callback 伺服器端呼叫的函式或函式名。如果沒有指定本引數，將在URL中尋找options['queryField']做為callback的方法名.
 ...
 * @config {Function} [onfailure] timeOut設定後才生效，到達超時時間時觸發本函式
 * @remark
 * 如果url中已經包含key為“options['queryField']”的query項，將會被替換成callback中引數傳遞或自動生成的函式名。
 * @meta standard
...
/**
 * 透過請求一個圖片的方式令伺服器儲存一條日誌
 * @function
 * @grammar baidu.sio.log(url)
 * @param {string} url 要傳送的地址.
 ...
 * @remark
 * 該方法的實現與ecma-262第五版中規定的JSON.parse不同，暫時只支援傳入一個引數。後續會進行功能豐富。
 * @meta standard
 ...
 * @remark
 * 該方法的實現與ecma-262第五版中規定的JSON.stringify不同，暫時只支援傳入一個引數。後續會進行功能豐富。
 * @meta standard
